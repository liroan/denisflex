...
const convertDate = (distributors: IDistributors[]) => {
    ...
    return !obj ? <span> &#8210;</span> : `${new Date(obj.date).toLocaleString('ru-RU', {
        year: 'numeric',
        month: 'long',
        day: "numeric"
    })}`
...
const lineTitles: ILineContent[] = [
    {
        ...
        title: "Страны",
        value: "countries",
        converter: (countries: ICountry[]) => countries.map(c => c.country).join(', ')
    },
...
const Film: FC<FilmProps> = React.memo(({movieData, budget, distributors, filmId}) => {
    ...
    const {
        data: similarFilms,
        isFetching: similarFilmsLoading,
        error: similarFilmsError
    } = useGetSimilarMovieByIdQuery(filmId);
        ...
        <div className={styles.film}>
            <Container>
                <Container>
                    ...
                    <Switcher activeCategory={activeCategory} switcher={switcher}
                    ...
                    <div className={styles.film__similar}>
                        <SliderContainer items={similarFilms?.items}
                                         getSliderCard={(item: IMovieSimilar) => <SliderItemSimilarMovie movie={item}
                                                                                                         key={item.filmId}/>}
                                         isLoading={similarFilmsLoading}
                                         error={similarFilmsError}
                                         title={getTitleWithCount("Похожее кино", similarFilms?.total)}
                        />
                    </div>
